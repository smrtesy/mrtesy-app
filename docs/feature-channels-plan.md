...
- **שגיאות בלוג הקשורות** — ספירת `log_entries` עם `level='error'` המקושרות לפיצ'ר. תלוי בכך שהלוגים ניתנים לקישור: בגרסה ראשונה סינון לפי `screen_key`; קישור מדויק לפי `feature_id` יתווסף ככל שהלוגים יתייגו את הפיצ'ר. אם אין קישור — העוזר הזה מוסתר, ו"כמה זמן בבטא" נשאר העוגן.
