...
> **סטטוס:** מאושר ונבנה. גרסת-v1 של המנגנון קיימת בקוד; עיצוב המסך עודכן ונבנה
> ב-2026-08-07 (ראו "עדכון עיצוב" למטה).
...
## עדכון עיצוב — 2026-08-07 (הכרעות חנוך, גוברות על גוף המסמך)

12 הכרעות שהתקבלו בדיון עיצוב המסך `/admin/features`. **במקום שהן סותרות את
§4.4/§4.5/§7 למטה — הן גוברות** (הגוף נשמר לתיעוד ההיגיון המקורי):

1. **בורר-ערוץ למשתמש** נשאר ברשימת `/admin/users` (לא במסך פרטי-משתמש).
2. **"כוונה" (fork/migrate) הוסרה לגמרי** — פיצול/החלפה הם *תוצאה* של מה שקורה
   לגרסאות, לא קלט. אזהרת תיקון-כפול וספירת §9 נגזרות מהמצב "שתי גרסאות חיות"
   (יציב≠בטא), בלי תווית ידנית.
3. **"קדם עד" (promote_by) הוסר** — אין תזכורת-תאריך.
4. **"קישור להסבר" (notes_url) הוסר** → הוחלף ב**הסבר-לכל-גרסה שקלוד כותב**
   (במרשם, כשדה `versions[].summary`), נגיש מהמסך; ל-STATE נוספה **הערה ידנית**
   (`feature_channels.note`).
5. **כפתור "קדם לרזה" הכפול הוסר** — נשאר טוגל בלבד (הטוגל *הוא* הפעולה).
6. **בחירת גרסה = בורר-רשימה** מהגרסאות שבמרשם, לא טקסט חופשי; מוסתר כשיש
   גרסה אחת בלבד.
7. **תצוגה קומפקטית**: שורה אחת לכל פיצ'ר (יציב: טוגל+גרסה · בטא: טוגל+גרסה);
   מתקפלת בעדינות בנייד. עשרות פיצ'רים למסך — קומפקטיות היא דרישה.
8. **היררכיה חלון=מסך**: כותרת-מסך עם **טוגל ראשי** שמכבה/מדליק את כל הפיצ'רים
   במסך בבת-אחת, ותחתיה שורות הפיצ'רים.
9. **מגירת היסטוריה מקופלת ברמת הפיצ'ר**: `תאריך · גרסה · תיאור קצר`, מוזנת
   מ-`versions[]` שבמרשם. באותה מגירה גם שדה ההערה הידנית.
10. **מודל שמירה מפורש** לשינויים קריטיים: טוגל/גרסה נצברים כ"ממתין", שורת-שמירה
    נדבקת למעלה (`N שינויים · שמור · בטל`), וחלים **רק** בשמירה. ההערה נשמרת
    בנפרד (טקסט חופשי, לא קריטי).
11. **גרסאות + הסבר + היסטוריה — קלוד כותב** במרשם בכל נגיעה בקוד (structure);
    המסך רק מנתב איזו גרסה בכל ערוץ (state).
12. **כפתור-אייקון "פתח מסך"** בכל שורת-פיצ'ר → פותח את מסך-הפיצ'ר בטאב-workspace
    חדש (`OpenTabLink`), בגרסת הערוץ של הצופה.

**קבצים שנגעו במימוש הזה:** `src/lib/feature-registry.ts` (+ twin
`server/src/lib/feature-registry.ts`) — הוסף `versions[]`, הוסרו `intent`/
`hasVersions`; `supabase/migrations/20260807160000_feature_channels_note.sql` —
`ADD COLUMN note` (העמודות `intent`/`promote_by`/`notes_url` נשארו מתות בטבלה
הריקה, ניתן לנקותן בעתיד בכרטיס-אישור); `server/src/modules/admin/features/routes.ts`;
`src/components/admin/FeaturesClient.tsx` (שכתוב מלא). ה-hook לא הושפע (אינו תלוי
ב-intent).
