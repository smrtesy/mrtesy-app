...
### 5. השעון — בתוך רשימת הצ'טים

התור חי **בתוך רשימת הצ'טים** (ה-rail), לא בפאנל נפרד. כשתיקון מגיע ל-`ready` הוא
מצטרף לקטגוריה חדשה בראש הרשימה — **ממתין למיזוג** — וכל הצ'טים הממתינים לפריסה
מרוכזים תחתיה. לצד כותרת הקטגוריה יושב **שעון ספירה-לאחור אחד** לכל הצרור:
`פורס בעוד M:SS (או כשהצרור יתייצב)`, גלוי גם כשעוד לא הגיע שקט. צ'אט במצב
`building` נשאר צ'אט פעיל רגיל ברשימה; הוא עובר לקטגוריה `ממתין למיזוג` רק כשהתיקון
שלו `ready`. קומפקטי-כברירת-מחדל: מרחיב את הרשימה הקיימת, לא סרגל חדש.
...
## בדיקת-איכות — מתי (פרוטוקול לפני-הדחיפה)

השער שמוגדר בריפו (build + greps ממוקדים + סקירת סוכן-משנה) **לא מדולג** — הוא רץ
בשתי רמות:

1. **לכל תיקון, לפני `mark-ready`:** הריצה שסיימה תיקון מריצה את **פרוטוקול
   לפני-הדחיפה המלא** על הענף שלה — בדיוק אותו שער של היום, רק שזז מ"לפני דחיפה
   ל-`main`" ל"לפני הכניסה לתור". רק אם נקי היא נכנסת כ-`ready`; אחרת היא מתקנת
   ולא מצטרפת לצרור.
2. **על הצרור הממוזג, לפני הדחיפה הסופית:** ריצת-הפריסה ממזגת את כל הענפים ומריצה
   **בנייה אחת** על התוצאה הממוזגת — בדיקת-אינטגרציה שתופסת שבירות שנוצרה
   *מהמיזוג* עצמו (שני תיקונים תקינים לחוד ששוברים יחד). לא חוזרים על סקירת-הסוכן
   של כל תיקון — היא כבר רצה לכל אחד ברמה 1. בנייה ממוזגת שנכשלת → הצרור `failed`,
   בלי דחיפה, עם פלט-הבנייה (ראה למטה).
...
1. **מיגרציה + נקודות-קצה + טבלת התור** (תוספתי, בלתי-נראה). — **נשלח 4/8** (commit `70d59f39`).
2. **שער-הדחיפה + עדכון `CLAUDE.md`** — לנתב ענפי `server/**` ל-`mark-ready` במקום
   לדחוף ל-`main`, **וגם** לעדכן את פרוטוקול לפני-הדחיפה ב-`CLAUDE.md` כך שלשינויי-שרת
   הוא רץ **לפני הכניסה לתור** (במקום "לפני דחיפה ל-`main`"); שינויי פרונט/דוקס — ללא
   שינוי, נדחפים ל-`main` ישירות אחרי הפרוטוקול. `CLAUDE.md` הוא נקודת-האכיפה שכל
   סשן קורא, ולכן העדכון שלו חייב לנחות **יחד** עם המנגנון — לא לפניו.
...
**שלבים 2 ו-3 חייבים לעלות יחד** (תיקון לניסוח קודם): שער-הדחיפה בלי המתאם ישאיר
תיקוני-שרת תקועים ב-`ready` בלי שאיש יפרוס אותם — כלומר יעצור את כל פריסות-השרת. שלב
1 עומד לבד (תוספתי, בלתי-נראה); 4 ו-5 הם שיפורי-תזמון/ממשק מעל 2+3 ואפשר לשלב אחריהם.

## החלטות והכרעות

- **[הוכרע 4/8]** חלון-ההתייצבות: **3 דקות** — מתחילים ככה, נעריך אם צריך.
- **[הוכרע 4/8]** בית השעון: **בתוך רשימת הצ'טים**, קטגוריה `ממתין למיזוג` עם שעון
  ספירה-לאחור לצד הכותרת (§5).
- **[הוכרע 4/8]** מי דוחף: **ריצת-פריסה ייעודית** (A) — הפרדת מוח/ידיים, עמיד יותר.
- **[פתוח]** **בניית-צרור שנכשלת** — לאתר איזה ענף שבר (bisect) או פשוט להכשיל את
  הצרור ולתת למשתמש להסתכל? (מומלץ: להכשיל + להציג קודם; bisect בהמשך אם יתברר
  מעצבן.)
