...
### CRITICAL — ONBOARDING בחלקים

בגלל מגבלות זמן ו-context, **אסור לסרוק את כל 45 הימים בריצה אחת**.

**אסטרטגיה — סריקה מהחדש לישן, בחלונות של 3 ימים:**

בתחילת כל ריצת ONBOARDING של PART1:
1. בדוק ב-Processing Log מה התאריך **הישן ביותר** שכבר עובד
2. הגדר חלון של 3 ימים שמסתיים בתאריך הזה (או "היום" אם הלוג ריק)
3. סרוק **רק את 3 הימים האלה** — לא יותר
4. בסוף הריצה עדכן ב-Run Session `Summary`: "עבד חלון X עד Y, נותרו Z ימים ל-onboarding"

ריצה הבאה של PART1 (שעה אחר כך) תמשיך אוטומטית מהמקום שעצרה.

אחרי ~15 ריצות (45÷3) ה-ONBOARDING יסתיים. אז הוא עובר למצב "ריצה רגילה".

**זיהוי שה-onboarding הסתיים:** הישן ביותר ב-Processing Log הוא יותר מ-45 יום אחורה.
