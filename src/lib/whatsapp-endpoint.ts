 ...
 *
 * By default every outbound WhatsApp Cloud API call (send message, download
 * media) goes DIRECTLY to Meta at graph.facebook.com, with the connection's Meta
 ...
 *
 * Twin file: server/src/lib/whatsapp-endpoint.ts (Railway/Express runtime). Keep
 * the two in sync — same env vars, same defaults. This env must be set in BOTH
 * Railway (server) and Vercel (these Next route handlers) for the flip to be
 * complete.
 */
