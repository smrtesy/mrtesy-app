 ...
 *
 * Outside a pane it's a plain <Link> — so on mobile, or on a full page with no
 * workspace open, it just navigates normally. This keeps in-page jumps (e.g. the
...
export function OpenTabLink({
  ...
  return (
    <Link
      ...
      className={className}
      onClick={(e) => {
        // Only intercept inside a pane; let normal navigation happen otherwise.
        if (isEmbeddedPane()) {
          e.preventDefault();
