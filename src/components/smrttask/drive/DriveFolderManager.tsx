...
/**
 * Manages the user's selected Drive folders (read + add + remove).
 * Used both on the Settings page and on the onboarding step. The
 * persistence target is `user_settings.drive_folder_ids` via PATCH
 * /api/me/settings — both surfaces write to the same source of truth.
 *
 * Emits `onChange(count)` after every successful write so the host
 * (the onboarding step) can enable/disable a "Continue" button based
 * on whether the user actually picked anything.
 */
export function DriveFolderManager({ onChange }: { onChange?: (count: number) => void }) {
