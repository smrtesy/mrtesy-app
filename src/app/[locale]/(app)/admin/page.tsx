...
export default async function AdminDashboard({
  ...
  const { data: syncStates } = await supabase
    .from("sync_state")
    .select("user_id, source, last_synced_at, consecutive_failures, last_error")
  ...
  function getServiceStatus(source: string) {
    ...
    const failed = states.filter((s) => s.consecutive_failures >= 5);
    const warn = states.filter((s) => s.consecutive_failures > 0 && s.consecutive_failures < 5);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {users.filter((u) => u.onboarding_completed).length} active
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${deadLetters > 0 ? "text-red-500" : ""}`}>
              {deadLetters}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${recentErrors.length > 0 ? "text-red-500" : ""}`}>
              {recentErrors.length}
            </div>
          </CardContent>
        </Card>
            ...
            return (
              <div key={service} className="flex items-center justify-between rounded-lg border p-3">
                ...
                <span className="text-xs text-muted-foreground">
                  {lastSync ? new Date(lastSync.last_synced_at).toLocaleString() : "Never"}
                </span>
              </div>
            );
          })}
      ...
      <div className="flex gap-3">
        <Link href={`${basePath}/users`} className="rounded-lg border p-3 hover:bg-accent flex-1 text-center text-sm font-medium">
          {t("users")}
        </Link>
        <Link href={`${basePath}/services`} className="rounded-lg border p-3 hover:bg-accent flex-1 text-center text-sm font-medium">
          {t("services")}
        </Link>
        <Link href={`${basePath}/logs`} className="rounded-lg border p-3 hover:bg-accent flex-1 text-center text-sm font-medium">
          {t("logs")}
        </Link>
      </div>
    </div>
