...
if [ "$STOP_ACTIVE" != "true" ]; then
  ...
  jq -cn --arg hint "$IDENTITY_HINT" '{
    decision: "block",
    reason: ("לפני שאתה עוצר — סכם את הסשן ל-smrtTask (זה רץ על המנוי, ללא עלות API):" + $hint + "\n1) כתוב סיכום קצר בעברית: נושא, 2-3 משפטים מה נדון/נעשה, וצעד המשך מוצע. שמור קישורים עמוקים מלאים אם רלוונטי.\n2) הרץ בדיוק פקודה אחת (ארגומנט 4 = סטטוס אופציונלי in_progress|blocked|done; ארגומנט 5 = מזהה-משימה אופציונלי — העבר אותו אם הפרומפט/הלינק של המשימה נשא מזהה, וכך הדיווח נצמד לכרטיס גם אם הוא לא מסומן \"בעבודה\"):\n   .claude/hooks/post-session-summary.sh \"<נושא קצר>\" \"<סיכום 2-3 משפטים>\" \"<צעד המשך>\" \"<סטטוס>\" \"<מזהה-משימה אם ידוע>\"\n   (הימנע מתו גרש-כפול \" בתוך הטקסט; השתמש ״ אם צריך.)\n3) אם הפלט כולל \"ok\":true — עצור. אל תריץ שוב ואל תבצע פעולות נוספות.")
  }'
  exit 0
fi
