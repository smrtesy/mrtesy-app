...
// The user's IANA timezone (from user_settings.timezone). ALL clock reasoning —
...
function nowContextLine(tz: string): string {
  ...
  return `Current date/time (${tz}, the user's local timezone): ${date} ${time} (${weekday}). Treat this as "now" when reasoning about deadlines, "today/tomorrow", and whether a stated time window has already passed. ALL [INCOMING]/[OUTGOING] timestamps in the body are ALSO in this timezone — when you write any time in title/description, write it in this local timezone, never UTC.`;
}
...
async function appendUpdateToTask(
  ...
  const { data: existing } = await supabase
    .from("tasks")
    .select("updates, status, title_he, task_type, manually_verified, due_date, recurrence_rule, recurrence_parent_id")
    .eq("id", taskId)
    .single();
  ...
  const deferToFollowup = isConversational(msg)
  ...
  if (deferToFollowup) {
    ...
    // A task the user already COMPLETED that a re-send revives must NOT jump back
    // onto the active desk unannounced — surface it as pending_completion so the
    // user re-confirms it's still done (the tuition-reminder case: "you brought
    // back a task I finished yesterday without asking, and didn't even mark it
    // for my approval"). Matters the user dismissed/archived (never "finished")
    // resurface active as before.
    if (existing?.status === "completed") {
      updateFields.status = "pending_completion";
      updateFields.completion_signal_detected = true;
      updateFields.completion_signal_reason = analysis.reason || "המשימה סומנה כהושלמה — התקבלה הודעה חדשה בנושא, נדרש אישור מחדש";
    } else {
    ...
    // Hybrid auto-close policy (option ג). A SYSTEM-GENERATED follow-up
    // tracker (task_type="followup") that the classifier now reports resolved
    // is closed AUTOMATICALLY — it was never a user action card, so it must not
    // linger as a pending_completion card the user has to click, nor bounce
    ...
    if (existing?.task_type === "followup") {
      ...
      updateFields.dismissal_reason_text = "העניין נסגר — המעקב נסגר אוטומטית";
      updateFields.completion_signal_detected = true;
...
A line like "(המשתמש הגיב 👍 על ההודעה הקודמת — אישור/סגירה, לא הודעה חדשה)"
is a REACTION (emoji) the user placed on the other party's message. Treat it as
the user ACKNOWLEDGING what the other side just sent — it counts as the user
having responded/closed that point, NOT as silence. (The "שלחתי שלוש thumb up,
למה זה נקרא שלא עניתי?" correction: a 👍 on delivered content IS a reply.)
Decide by the LAST meaningful exchange, in this priority order (first match wins):
  1. The OTHER party committed to do / check / answer something that HAS NOT
     ...
     politely ("תודה", "👍", "מעולה"). A polite acknowledgement of a still-open
     PROMISE does not cancel the need to track it; only the promised thing
     actually ARRIVING does.
     CRUCIAL DISTINCTION — promise vs. delivery: if the other party already
     DELIVERED the thing (sent the materials / images / file / answer / quote)
     and the user acknowledged it (👍 reaction, "תודה", "קיבלתי", "מעולה"),
     the loop is CLOSED → INFORMATIONAL, completion=true. Do NOT open a
     "follow up with X" task for something the user already received and
     acknowledged.
...
async function createTasksFromMessage(msg: any, sys: SystemParams, settings: any, userId: string, projectContext?: { projectId: string; brief: string }, modelOverride?: string, waHighWater?: string | null) {
  ...
  // Mandatory action-link addendum, appended for the same reason as the
...
async function whatsappChatSiblingIds(userId: string, chatId: string, sourceType = "whatsapp"): Promise<string[]> {
  // Gather sibling burst rows for a chat, scoped to the SAME channel — a
  // WhatsApp chatId and an SMS peer phone could coincide, so never mix them.
...
async function tryLinkToExistingTask(msg: any, userId: string): Promise<{ id: string; updates: any[] } | null> {
  ...
  if (msg.source_type === "whatsapp" || msg.source_type === "sms") {
    ...
    const sibIds = await whatsappChatSiblingIds(userId, chatId, msg.source_type);
...
async function processMessage(msg: any, settings: any, sys: SystemParams) {
  ...
  // ── Unsupported / unavailable WhatsApp placeholder gate ───────────────────
  // When WhatsApp can't surface a message to the Cloud API the webhook stores a
  // placeholder body ("[הודעה לא נתמכת… בדוק בטלפון]" / "[הודעה לא זמינה…]").
  // That placeholder is not readable content — turning it into an actionable
  // "בדוק בטלפון" suggestion, or letting it REVIVE a matter the user already
  // dismissed, is pure noise (the "אני לא רואה שום הודעה" / "החזיר משימה ישנה
  // של רסקין" corrections). If the LATEST message is ONLY such a placeholder,
  // drop to informational and skip matter-routing entirely — no task, no
  // revival, no matter update. A user override still forces it through.
  const waLatestBody = String(msg.body_text ?? "").trim();
  // NOTE: no \b after the word — JS/V8 (and Deno) define \b on ASCII only, so
  // there is no word boundary after a Hebrew letter and \b would make this never
  // match the real placeholders ("[הודעה לא נתמכת: …]" / "[הודעה לא זמינה …]").
  const isUnsupportedPlaceholder =
    isWhatsApp(msg) &&
    /^\[הודעה לא (נתמכת|זמינה)/.test(waLatestBody);
  if (isUnsupportedPlaceholder && !userForceActionable) {
    classification = "informational";
    classificationReason = `whatsapp: unsupported/unavailable placeholder ("${waLatestBody.slice(0, 40)}") — not readable content, skipped task/routing`;
  }
  ...
  const whatsappRoutingActive = sys.whatsapp_matter_routing && isConversational(msg);
  ...
  // An unsupported/unavailable placeholder carries no content to route or to
  // attach to a matter, so it bypasses routing entirely (gate above).
  if (whatsappRoutingActive && !isUnsupportedPlaceholder && (classification === "actionable" || classification === "informational")) {
    try {
  ...
  if (!linkedTaskId && classification === "actionable" && !userForceActionable && isConversational(msg)) {
    ...
    try {
      ...
      if (calendarForceActionable) {
        ...
        if (taskResult.tasks.length === 0) {
          ...
          // Correction #6b — anchor each WhatsApp task to the SPECIFIC chat
          // message its matter was mined from, not msg.id (the latest burst
          // row), which is often an unrelated later line (the שוויגער case: a
          // card-update task anchored to a group-invite link sent minutes
          // later). The builder emits `source_quote` — a verbatim snippet of
          // the anchoring line — and we match it against the chat's sibling
          // rows by TEXT (tz-independent, unlike the local-vs-UTC timestamps).
          // Any failure to match falls back to msg.id, i.e. today's behaviour.
          let waAnchorRows: { id: string; body_text: string | null }[] = [];
          if (msg.source_type === "whatsapp" && msg.metadata?.chatId) {
            const { data: sibs, error: sibErr } = await supabase
              .from("source_messages")
              .select("id, body_text, received_at")
              .eq("user_id", msg.user_id)
              .eq("source_type", "whatsapp")
              .filter("metadata->>chatId", "eq", String(msg.metadata.chatId))
              .lte("received_at", msg.received_at)
              .order("received_at", { ascending: false })
              .limit(40);
            if (sibErr) console.error("wa anchor sibling fetch failed:", sibErr);
            waAnchorRows = sibs ?? [];
          }
          const normAnchor = (s: string) => s.replace(/\s+/g, " ").trim().toLowerCase();
          const resolveWaAnchor = (quote: unknown): string => {
            if (msg.source_type !== "whatsapp" || typeof quote !== "string") return msg.id;
            const q = normAnchor(quote).slice(0, 60);
            // Too-short quotes match promiscuously ("כן", "ok") — not reliable.
            if (q.length < 8) return msg.id;
          ...
          for (const task of taskResult.tasks) {
            ...
            const { data: newTask, error: taskInsertError } = await supabase.from("tasks").insert({
              user_id: msg.user_id, source_message_id: resolveWaAnchor(task.source_quote),
              ...
              ai_actions: task.ai_actions || [],
              // Action nuggets — sanitise the model's output to a well-formed
              // [{label,url}] array of real http(s) links, capped, so a malformed
              // reply can never store junk in the column the UI reads.
              action_links: Array.isArray(task.action_links)
                ? task.action_links
                    .filter((a: any) => a && typeof a.url === "string" && /^https?:\/\//i.test(a.url))
                    .map((a: any) => ({ label: typeof a.label === "string" ? a.label : "", url: a.url }))
                    .slice(0, 10)
                : [],
              related_contact: task.owner_contact,
